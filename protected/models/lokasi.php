...
class Lokasi extends CActiveRecord
{
	...
	public function getSub()
	{
		...
		foreach ($query as $key => $value) {
			$data[] = array(
				...
				'url'=>array('/lokasi/index', 'kota'=>$value['id']), 
			);
		}
		// print_r($query);
		// exit;
	...
	public function getKota($state)
	{
		...
		foreach ($kota as $key => $value) {
			$kotaArray[]=array('label'=>$value->city_name, 'url'=>array('/lokasi/index', 'kota'=>$value->id, 'state'=>$state));
		}
		return $kotaArray;
	}
	public function getLokasi($kota)
	{
		if ($kota=='') {
			$kota = $this->findAll();
		} else {
