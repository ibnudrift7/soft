...
class UserController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['User']))
		{
			...
			if ($model->validate()) {
				...
				$model->save();
				Log::createLog("User Create $model->id $model->user");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['User']))
		{
			...
			if ($model->validate()) {
				...
				$model->save();
				Log::createLog("User Update $model->id $model->user");
				$this->redirect(array('view','id'=>$model->id));
			}
		}
		$model->pass = '';
	...
	{
		$model=User::model()->find('user = :user', array(':user'=>Yii::app()->user->id));
		...
		if(isset($_POST['User']))
		{
			...
			if ($model->validate()) {
				...
				if (sha1($_POST['User']['passold'])==$pass AND $_POST['User']['pass']==$_POST['User']['passconf']) {
					$model->pass = sha1($_POST['User']['pass']);
					$model->save();
					Log::createLog("User Update Pass $model->id $model->user");
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			Log::createLog("User Delete $data->user $data->id");
			$data->delete();
