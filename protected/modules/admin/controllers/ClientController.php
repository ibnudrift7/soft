	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Client']))
		{
			...
			if($model->validate()){
				...
				try
				{
					$img->saveAs(Yii::getPathOfAlias('webroot').'/images/client/'.$model->images);
					$model->sort = ($model->count()+1);
					$model->save();
					Log::createLog("ClientController Create $model->id");
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Client']))
		{
			...
			if($model->validate()){
				...
				{
					if($img->name!=''){//jika di edit
						$img->saveAs(Yii::getPathOfAlias('webroot').'/images/client/'.$model->images);//upload file
						@unlink(Yii::app()->basePath."/../images/client/".$images);
					}
					$model->save();
					Log::createLog("ClientController Update $model->id");
